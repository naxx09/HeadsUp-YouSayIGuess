...
class Main {
    ...
    getRandomList() {
        ...
        while (currentIndex < 30) {
            let x = Math.floor(Math.random() * this.input.length);
    ...
    // 2 mins countdown
    setCountdown() {
        let timer = 120, minutes: any, seconds: any;
        let spanTimer = document.getElementById("spanTimer");
    ...
    setTimer() {
        setTimeout(() => {
            this.toggle(document.getElementById("divGame"));
            this.toggle(document.getElementById("divEnd"));
            clearInterval(this.countInt);
        }, 120000);
    }
}
let mainClass = new Main(lib2);
...
class GuessSong implements Audio {
    name: string = "";
    singer: string = "";
    filename: string = "";
    x: number;
    last: number;
    constructor(public audioList: string[]) { }
...
let audioClass = new GuessSong(audioList);
let divSongsName = document.getElementById("divSongsName");
if (divSongsName) {
